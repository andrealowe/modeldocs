...
def resolve_domino_url() -> tuple[str, str]:
    for var in ("DOMINO_URL", "DOMINO_API_HOST"):
        val = os.environ.get(var, "")
        if val:
            return val, var
    return "https://cloud-dogfood.domino.tech", "default"
...
def register_models():
    ...
    with mlflow.start_run():
        ...
        pd_model_info = mlflow.pyfunc.log_model(
            artifact_path="GetLoanProbabilityOfDefault",
    ...
    with mlflow.start_run():
        ...
        inventory_model_info = mlflow.pyfunc.log_model(
            artifact_path="GetLoanInventory",
